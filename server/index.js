...
function rateLimit(req, res, next) {
  ...
  if (list.length >= 5) return res.status(429).json({ ok: false, error: 'rate_limited', message: 'Too many requests. Please try again in a minute.' });
  list.push(now);
  hits.set(ip, list);
  next();
}
...
function addLeadEvent({ leadId = null, eventType, payload = {}, requestId = null }) {
  try {
    ...
    console.warn('lead event insert skipped:', eventType, err?.message || err);
  }
}
...
app.post('/api/lead', rateLimit, (req, res) => {
  try {
    ...
    const result = insertLead.run(createdAt, value.name, value.email, value.phone, value.company, value.product, value.message || '', value.utm, value.lang, value.source_url, value.schema_version, LEAD_STATUS.NEW, ip, userAgent);

    const leadId = Number(result.lastInsertRowid);
    insertQueueItem.run(leadId, QUEUE_STATUS.PENDING, createdAt, createdAt, createdAt);
    addLeadEvent({ leadId, eventType: 'created', payload: { schema_version: value.schema_version }, requestId: req.requestId });
    addLeadEvent({ leadId, eventType: 'queued', payload: { status: QUEUE_STATUS.PENDING }, requestId: req.requestId });

    return res.status(200).json({ ok: true, id: leadId, schema_version: value.schema_version, status: LEAD_STATUS.NEW, request_id: req.requestId });
  } catch (err) {
